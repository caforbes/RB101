...
=begin
Problem: we perform integer division on two numbers, which frequently rounds
down to zero. We therefore don't get an accurate log measure, or TF-IDF. The
solution is to convert the integer numbers to floats before performing division.

Second, when dividing floats it is possible to divide by zero. This produces an
Infinity/NaN response for the TFIDF when the term cannot be found anywhere. A
simple check condition/guard clause will solve this.
=end
