...
=begin
- take an array of integers (pos/neg)
- return index at which elements on both sides have equal sums
- don't count the value of element at that index
- if a balanced index can't be found return -1

- given an index, get sum of elements before and after
  - get two sub-arrays of an array based on an index: before/after elements
    - first half: [0...ind]
    - second half: [ind+1..-1]
  - transform subarrays to their sums
- if before/after sums are equal, return the index
- if the end is reached without equivalence, return -1
=end

def find_even_index(array)
  array.each_index do |index|
    before_sum = array[0...index].sum
    after_sum = array[index+1..-1].sum
    return index if before_sum == after_sum
  end
  -1
end
